...
FitnessOS follows a domain-oriented logical database modeling approach.

Each domain maintains its own authoritative `LOGICAL_DATABASE_MODEL.md` describing its logical entities, relationships, cardinality, lifecycle dependencies, and cross-domain references.

The enterprise Logical Database Model defines the standards, modeling principles, relationship classifications, ownership rules, normalization strategy, and governance for logical data modeling across FitnessOS.

Domain-specific logical models SHALL NOT be duplicated within this document.

---

# Cross-Domain Reference Strategy

Cross-domain references SHALL be documented within the owning domain's logical model while adhering to the enterprise modeling standards defined in this document.

References SHALL preserve ownership boundaries and SHALL NOT transfer aggregate ownership.
---
...
- Logical database modeling standards are documented.
- Cross-domain reference strategy is defined.
- Lifecycle dependency rules are documented.
- Read/write separation is defined.
